function [a_vec,e_vec,i_vec,w_vec,Omega_vec,sigma_vec] = orbelem_averaged_J2(t_vec,a_0,e_0,i_0,w_0,Omega_0,sigma_0,mu,R_0,J_2)
%==========================================================================
%
% Calculates orbital elements for selected times with averaging theory.
% J2 perturbation.
% Program assumes that a stays constant throughout the times. 
% Equations obtained from Eq 11.17 from A. Roy (page 333).
% Assumes t0 = 0.
%
% Author: G. Montseny
% Date: October 29, 2025
%
%
% INPUT:               Description                                   Units
%
%  t_vec   -          input time  array                                s
%  a_0  -             semi-major axis  at t = 0                       km 
%  i_0 -              inclination                                     deg
%  e_0  -             eccentricity                                     -
%  w_0 -              argument of periapsis                           deg
%  Omega_0 -          longitude of the ascending node                 deg 
%  sigma_0  -        time of periapsis                                 s
%  mu   -           mass parameter                                 km^3/s^2
% R_0               - radius of   planet                          km
% J_2               -   constant
%
% OUTPUT:       
%    
%  a_vec  -             averaged semi-major axis  at t                   km 
%  i_vec -              averaged inclination at t                       deg
%  e_vec  -             averaged eccentricity at t                       -
%  w_vec -              averaged argument of periapsis  at t            deg
%  Omega_vec -          averaged longitude of the ascending node  at t  deg 
%  sigma_vec  -         averaged time of periapsis at t                  s
%==========================================================================

% Preallocate all output arrays
n_t = length(t_vec);
a_vec = zeros(1,n_t);
e_vec = zeros(1,n_t);
i_vec = zeros(1,n_t);
w_vec = zeros(1,n_t);
Omega_vec = zeros(1,n_t);
sigma_vec = zeros(1,n_t);

% Calculate other orbital parameters
p_0 = a_0*(1-e_0^2);
n_0 = sqrt(mu/a_0^3);

% Calculate the averaged orbit elements at different times
for i = 1 : n_t
    t = t_vec(i);

    a_vec(i) = a_0;

    e_vec(i) = e_0;

    i_vec(i) = i_0;

    w_vec(i) = w_0 + 1.5*J_2*R_0^2*n_0*(2-5*sind(i_0)^2/2)*t/p_0^2;

    Omega_vec(i) = Omega_0 - 1.5*J_2*R_0^2*n_0*cosd(i_0)*t/p_0^2;

    sigma_vec(i) = sigma_0 + 1.5*J_2*R_0^2*sqrt(1-e_0^2)*n_0*(1-1.5*sind(i_0)^2)*t/p_0^2;
end

end